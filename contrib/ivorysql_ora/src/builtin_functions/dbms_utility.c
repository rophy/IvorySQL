...
/*
 * ora_format_error_backtrace_internal - Internal implementation
 *
 * This version takes the PostgreSQL context string as input and
 * transforms it to Oracle format. It will be called from the
 * PL/iSQL package body.
 */
PG_FUNCTION_INFO_V1(ora_format_error_backtrace_internal);

Datum
ora_format_error_backtrace_internal(PG_FUNCTION_ARGS)
{
	text *pg_context_text;
	char *pg_context;
	StringInfoData result;
	char *context_copy;
	char *line;
	...
	pg_context_text = PG_GETARG_TEXT_PP(0);
	pg_context = text_to_cstring(pg_context_text);

	/* If context is empty, return NULL */
	if (pg_context == NULL || pg_context[0] == '\0')
		PG_RETURN_NULL();
